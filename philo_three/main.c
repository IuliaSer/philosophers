...
/*   Updated: 2021/01/22 13:10:56 by iserzhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		free_all(t_all *all)
{
	...
	sem_close(all->data->forks);
	sem_unlink("/semaphor_write");
	sem_close(all->data->write_lock);
	if (all->phil)
		free(all->phil);
	all->phil = NULL;
}
...
void		init_vars(char **argv, int argc, t_all *all)
{
	...
	sem_unlink("/semaphor_death");
	all->data->write_lock = sem_open("/semaphor_death", O_CREAT, 0666, 1);
}
